"""Single settings source for RagFlowCache.
...
class Settings(BaseSettings):
    ...
    database_url: str = "postgresql://ragflow:ragflow@localhost:5432/ragflowcache"
    ...
    # Cache augmented generation controls. The whole corpus is preloaded into
    # the prompt, capped by max_context_chars so it fits the context window. A
    # new question hits the cache when its cosine similarity to a past question
    # is at or above the threshold, returning the stored answer with no model
    # call at all.
